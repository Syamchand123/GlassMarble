...
func TestGraftFileNodeCreatesTree(t *testing.T) {
	...
	gastRoot := &normalize.GASTNode{Type: normalize.GASTFileRoot, Name: "c.go"}
...
func TestGraftFileNodeRootFile(t *testing.T) {
	root := NewDirectoryNode("root", ".")
	gastRoot := &normalize.GASTNode{Type: normalize.GASTFileRoot, Name: "main.go"}
...
func TestGraftFileNodeOverwritesExistingFile(t *testing.T) {
	root := NewDirectoryNode("root", ".")
	GraftFileNode(root, "a/b/c.go", &normalize.GASTNode{Type: normalize.GASTFileRoot, Name: "c.go"}, []string{"fmt"}, "go")

	second := &normalize.GASTNode{Type: normalize.GASTFileRoot, Name: "c2.go"}
...
func TestGraftFileNodeInvalidInputs(t *testing.T) {
	...
	assert.NotPanics(t, func() {
		GraftFileNode(nil, "a/b/c.go", &normalize.GASTNode{}, nil, "go")
		GraftFileNode(root, "", &normalize.GASTNode{}, nil, "go")
	})
	assert.Empty(t, root.SubFolders)
...
func TestPruneFileNode(t *testing.T) {
	root := NewDirectoryNode("root", ".")
	GraftFileNode(root, "a/b/c.go", &normalize.GASTNode{Type: normalize.GASTFileRoot, Name: "c.go"}, nil, "go")
...
func TestPruneLeavesSibling(t *testing.T) {
	root := NewDirectoryNode("root", ".")
	GraftFileNode(root, "a/b/c.go", &normalize.GASTNode{Type: normalize.GASTFileRoot, Name: "c.go"}, nil, "go")
	GraftFileNode(root, "a/b/d.go", &normalize.GASTNode{Type: normalize.GASTFileRoot, Name: "d.go"}, nil, "go")
