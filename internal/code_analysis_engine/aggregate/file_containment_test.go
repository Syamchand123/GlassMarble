...
func runAggregate(t *testing.T, files map[string]string) *AggregateOutput {
	t.Helper()
	dir := t.TempDir()
...
func TestFileContainment(t *testing.T) {
	out := runAggregate(t, fixtureFiles(t))
...
func TestOwnershipMap(t *testing.T) {
	...
	// dispatch, find, render — all re-parented by the normalizer.
