...
// It tags offending calls with "EdgeCycleViolation" primitive to alert the linker and Visualization.
func DetectArchitecturalCycles(queue []LinkedCallSite) {
	...
	for _, call := range queue {
		...
		// We use receiver or file hint to guess target folder at this phase
		// Since we don't have perfect linking yet, this is a coarse heuristic
	...
	for i := range queue {
		...
		if cycleEdges[srcFolder] != nil && cycleEdges[srcFolder][tgtFolder] {
			...
			for _, p := range call.Primitives {
				if p == normalize.PrimCycleViolation {
					found = true
					break
				}
			}
			if !found {
				call.Primitives = append(call.Primitives, normalize.PrimCycleViolation)
			}
		}
	}
}
...
func guessTargetFolder(call LinkedCallSite) string {
	// A rough heuristic for architectural bounds at Aggregation
