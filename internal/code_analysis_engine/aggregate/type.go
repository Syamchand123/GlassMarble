...
// AggregateOutput represents the Aggregated Physical System Topology emitted by Aggregation.
type AggregateOutput struct {
	...
	GlobalDefinitionIndex map[string][]*normalize.GASTNode `json:"global_definition_index"`

	// GlobalCallQueue is a unified bucket containing every unresolved external call site in the system.
	// This acts as a targeted checklist for the Interprocedural Linker.
	...
	// structural member defined in that file. the linker consumes this to emit
	...
	// LocalTables preserves the file-level semantic mappings (Step 2.3 output) for the linker.
	LocalTables map[string]*normalize.FileSymbolTable `json:"local_tables"`
	...
	ExternalDependencies map[string]*normalize.GASTNode `json:"external_dependencies"`
...
type FileBoundaryNode struct {
	...
	GASTRoot     *normalize.GASTNode `json:"gast_root"`     // Normalized AST tree from Normalization
	LocalImports []string         `json:"local_imports"` // List of imports declared in this file
}

// LinkedCallSite represents an unresolved call site waiting for the graph linker.
type LinkedCallSite struct {
	...
	Primitives       []normalize.BehavioralPrimitive `json:"primitives"`          // Behavioral flags
