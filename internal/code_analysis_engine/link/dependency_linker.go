...
func LinkFileDependencies(aggregateOut *aggregate.AggregateOutput, cpg *LinkOutput) {
	if aggregateOut == nil || aggregateOut.RootNode == nil || cpg == nil {
		return
	}

	var files []fileEntry
	collectFileImports(aggregateOut.RootNode, &files, cpg)
	...
	for _, fe := range files {
		...
		for _, imp := range fe.imports {
			...
			if aggregateOut != nil && !aggregate.IsStdlibImport(imp, fe.relPath) {
				if extNode, ok := aggregateOut.ExternalDependencies[aggregate.ResolveExternalKey(imp)]; ok {
...
func collectFileImports(dir *aggregate.DirectoryNode, out *[]fileEntry, cpg *LinkOutput) {
	if dir == nil {
		return
	}
	...
	for _, file := range dir.Files {
		...
		*out = append(*out, fileEntry{
			relPath: aggregate.NormalizeRelativePath(file.RelativePath),
...
func detectCyclicDependencies(cpg *LinkOutput) {
	index := 0
	stack := []string{}
