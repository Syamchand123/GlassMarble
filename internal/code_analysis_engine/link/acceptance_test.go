...
// Helper: build a AggregateOutput from a map of filename->source.
func buildAggregateFromSource(t *testing.T, sources map[string]string) *aggregate.AggregateOutput {
	t.Helper()
	dir := t.TempDir()
	...
	out, err := ingest.RunIngestion(ingest.Config{RootDir: dir})
	if err != nil {
		t.Fatalf("RunIngestion: %v", err)
	}
	payload, err := normalize.Normalize(out, "acceptance")
	if err != nil {
		t.Fatalf("Normalize: %v", err)
	}
	s3out, err := aggregate.Aggregate(payload, nil, "")
	if err != nil {
		t.Fatalf("aggregate.Aggregate failed: %v", err)
	}
	return s3out
}

// Helper: run the full linker on a AggregateOutput with given modified files.
func link(t *testing.T, aggregateOut *aggregate.AggregateOutput, modified []string) *LinkOutput {
	t.Helper()
	out, err := Link(aggregateOut, modified, nil)
	if err != nil {
...
func countEdgeType(cpg *LinkOutput, typ RelationshipType) int {
	n := 0
...
func hasEdgeType(cpg *LinkOutput, typ RelationshipType) bool {
	for _, edges := range cpg.OutboundEdges {
		for _, e := range edges {
			if e.Type == typ {
				return true
...
`,
	...
	aggregateOut := buildAggregateFromSource(t, sources)
	cpg := link(t, aggregateOut, []string{"order.go"})
...
`,
	...
	aggregateOut := buildAggregateFromSource(t, sources)
	cpg := link(t, aggregateOut, []string{"mix.go"})
...
`,
	...
	aggregateOut := buildAggregateFromSource(t, sources)
	cpg := link(t, aggregateOut, nil) // empty ModifiedFiles => full rebuild
...
`,
	...
	aggregateOut := buildAggregateFromSource(t, sources)
	cpg := link(t, aggregateOut, []string{"svc.go"})
...
func TestAllEdgeConstantsProduced(t *testing.T) {
	...
	// Gather all non-test Go files in the link package (excluding type.go itself, as it only contains declarations).
	...
	if len(prodFiles) == 0 {
		t.Fatalf("no production files found in link")
	}
