...
func LinkIntraProceduralControlFlow(aggregateOut *aggregate.AggregateOutput, cpg *LinkOutput) {
	...
	traverseForCFG(aggregateOut.RootNode, cpg, aggregateOut, om, branchCounts)
...
func traverseForCFG(dir *aggregate.DirectoryNode, cpg *LinkOutput, aggregateOut *aggregate.AggregateOutput, om *aggregate.OwnershipMap, branchCounts map[string]map[string]int) {
	if dir == nil {
		return
	}
	...
	for _, file := range dir.Files {
		...
		extractCFGNodesFromGAST(file.GASTRoot, file.RelativePath, "", cpg, aggregateOut, om, branchCounts, fileNodeCount)
	}

	for _, subDir := range dir.SubFolders {
		traverseForCFG(subDir, cpg, aggregateOut, om, branchCounts)
	}
}

func extractCFGNodesFromGAST(node *normalize.GASTNode, relPath, currentEnclosingFuncID string, cpg *LinkOutput, aggregateOut *aggregate.AggregateOutput, om *aggregate.OwnershipMap, branchCounts map[string]map[string]int, fileNodeCount map[string]int) {
	if node == nil {
		return
	}
	...
	if enclosingFuncID != "" && node.Type == normalize.GASTControlFlow {
		if branchCounts != nil {
			...
			if branchKind != "" {
				...
				for _, child := range node.Children {
					if child.Type == normalize.GASTCallExpression {
						targetCallFQN, _ := resolveCallTarget(child.ReceiverType, child.Name, relPath, nil, om, cpg, aggregateOut)
						if targetCallFQN != "" {
...
skipCFGNode:
	for _, child := range node.Children {
		extractCFGNodesFromGAST(child, relPath, enclosingFuncID, cpg, aggregateOut, om, branchCounts, fileNodeCount)
	}
}

// buildCFGSummaryNodes creates one CFG_SUMMARY node per function with branch type counts.
func buildCFGSummaryNodes(branchCounts map[string]map[string]int, cpg *LinkOutput) {
...
// tree-sitter kind strings set by the normalizer.
