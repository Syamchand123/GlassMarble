...
type LinkerConfig struct {
	...
	OwnershipMap *aggregate.OwnershipMap `json:"-"`
}

// LinkOutput is the final, completely bound code compilation payload (Complete Code Property Graph).
type LinkOutput struct {
	...
	// Phase 3.8: Root Execution Nodes (Endpoints, Mains)
	EntrypointRegistry []string `json:"entrypoints"`

	// Phase 3.9: Architectural Folder Zones (e.g., src/database -> DATABASE_ZONE)
...
func isFullMode(cpg *LinkOutput) bool {
...
func ownershipMap(cpg *LinkOutput, aggregateOut *aggregate.AggregateOutput) *aggregate.OwnershipMap {
	...
	return aggregate.BuildOwnershipMap(aggregateOut.GlobalDefinitionIndex, aggregateOut.WorkspaceCtx)
}

// SetDB attaches the read-only global graph database for incremental lookups.
func (s *LinkOutput) SetDB(db GraphDB) {
	s.db = db
}

// GetNode resolves a node by checking local delta, shared base, then global DB.
func (s *LinkOutput) GetNode(id string) (*ResolvedNode, bool) {
...
func (s *LinkOutput) NodeExists(id string) bool {
...
func (s *LinkOutput) HasNode(id string) bool {
	if _, ok := s.GraphNodes[id]; ok {
		return true
	}
	if s.baseNodes != nil {
...
func (s *LinkOutput) nameToNodeID() map[string]string {
...
// It enables the Incremental Delta Linker to lookup existing symbols in unmodified files.
...
// NewLinkOutput instantiates a fresh LinkOutput structure.
func NewLinkOutput(commitHash string) *LinkOutput {
	return &LinkOutput{
...
func (s *LinkOutput) AddEdge(sourceID, targetID string, edgeType RelationshipType, lineNo int) {
...
func (s *LinkOutput) AddEdgeProperties(sourceID, targetID string, edgeType RelationshipType, lineNo int, confidence float32, props map[string]string) {
	if sourceID == "" || targetID == "" || sourceID == targetID {
		return
	}
...
func (s *LinkOutput) registerEdge(edge ResolvedEdge) bool {
...
func ensureVirtualNode(id, kind, name string, cpg *LinkOutput) {
...
func (s *LinkOutput) AddEdgeWithConfidence(sourceID, targetID string, edgeType RelationshipType, lineNo int, confidence float32) {
	if sourceID == "" || targetID == "" || sourceID == targetID {
		return
	}
...
func touchesFile(edge ResolvedEdge, cpg *LinkOutput, filePath string) bool {
	norm := aggregate.NormalizeRelativePath(filePath)
	...
	check := func(id string) bool {
		...
		if n.FileSpec.Path != "" && aggregate.NormalizeRelativePath(n.FileSpec.Path) == norm {
			return true
		}
		if fp := n.Properties["file_path"]; fp != "" && aggregate.NormalizeRelativePath(fp) == norm {
			return true
		}
		return false
	}
...
// scoped to any file).
func (s *LinkOutput) RemoveNode(id string) {
	if s == nil {
		return
	}
