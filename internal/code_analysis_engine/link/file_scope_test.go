...
func TestFileToSymbolEdgesReal(t *testing.T) {
	...
	progDir := aggregate.NewDirectoryNode("internal", "internal")
	root.SubFolders["internal"] = progDir

	analyzeDir := aggregate.NewDirectoryNode("analyze", "internal/tui/programs/analyze")
	progDir.SubFolders["tui"] = aggregate.NewDirectoryNode("tui", "internal/tui")
	progDir.SubFolders["tui"].SubFolders["programs"] = aggregate.NewDirectoryNode("programs", "internal/tui/programs")
	progDir.SubFolders["tui"].SubFolders["programs"].SubFolders["analyze"] = analyzeDir

	analyzeDir.Files["program.go"] = &aggregate.FileBoundaryNode{
		FileName:     "program.go",
		...
		GASTRoot: &normalize.GASTNode{
			...
			Children: []*normalize.GASTNode{
				{Type: normalize.GASTField, Name: "TargetDir"},
				{Type: normalize.GASTFunction, Name: "Run", Kind: "method", ReceiverType: "Options"},
			},
		},
	}
