...
func TestBuildInitialNodesFull(t *testing.T) {
	...
	out := BuildInitialNodes(aggregateOut, nil)
		...
		RootNode: &aggregate.DirectoryNode{
			...
			Files: map[string]*aggregate.FileBoundaryNode{
				...
				"b.go": {
					FileName:     "b.go",
					RelativePath: "b.go",
					GASTRoot:     &normalize.GASTNode{Type: normalize.GASTFunction, Name: "other", Kind: "function"},
				},
			},
		},
	}

	out := BuildInitialNodes(aggregateOut, []string{"a.go"})
...
func TestLinkTypesExtendsAndComposes(t *testing.T) {
	...
	LinkTypesAndComposition(aggregateOut, cpg)
...
func TestLinkImplementsWhenMethodsSatisfied(t *testing.T) {
	...
	aggregateOut := &aggregate.AggregateOutput{CommitHash: "t", GlobalDefinitionIndex: map[string][]*normalize.GASTNode{}}
...
func TestResolveCallTargetStaticSameFile(t *testing.T) {
	...
	aggregateOut := &aggregate.AggregateOutput{CommitHash: "t", WorkspaceCtx: aggregate.NewWorkspaceContext()}

	id, conf := resolveCallTarget("Handler", "Process", "svc.go", nil, nil, cpg, aggregateOut)
...
func TestResolveCallTargetUnresolved(t *testing.T) {
	cpg := NewLinkOutput("t")
	aggregateOut := &aggregate.AggregateOutput{CommitHash: "t", WorkspaceCtx: aggregate.NewWorkspaceContext()}
	id, conf := resolveCallTarget("Missing", "Nope", "x.go", nil, nil, cpg, aggregateOut)
	if id != "" || conf != 0.0 {
...
func TestLinkCallGraphBasic(t *testing.T) {
	...
	callSite := aggregate.LinkedCallSite{
	...
	aggregateOut := &aggregate.AggregateOutput{
		CommitHash:      "t",
		GlobalCallQueue: []aggregate.LinkedCallSite{callSite},
		WorkspaceCtx:    aggregate.NewWorkspaceContext(),
	}
	...
	LinkCallGraph(aggregateOut, cpg)
...
func TestLinkFileDependencies(t *testing.T) {
	...
	LinkFileDependencies(aggregateOut, cpg)
...
func TestLinkSemanticEndpointsAndSinks(t *testing.T) {
	aggregateOut := &aggregate.AggregateOutput{
		...
		LocalTables: map[string]*normalize.FileSymbolTable{
			"api.go": {
				Endpoints:     []normalize.EndpointMeta{{Method: "GET", Route: "/api/users", LineNumber: 5}},
				SecuritySinks: []normalize.SecuritySinkMeta{{SinkType: "SQL", LineNumber: 6}},
				ResourceLinks: []normalize.ResourceMeta{{ResourceType: "FileSystem", LineNumber: 7}},
				GlobalState:   []normalize.SymbolMeta{{Name: "cache"}},
			},
		},
	}
	cpg := NewLinkOutput("t")
...
func TestLinkConcurrencyAndMessagePassing(t *testing.T) {
		...
		GlobalCallQueue: []aggregate.LinkedCallSite{
			{
				...
				MethodName:       "Process",
				LineNumber:       3,
				Primitives:       []normalize.BehavioralPrimitive{normalize.PrimConcurrency},
			},
			{
	...
	LinkConcurrencyAndAsyncControlFlow(aggregateOut, cpg)
...
func TestLinkEventSourcing(t *testing.T) {
	...
	LinkEventSourcing(aggregateOut, cpg)
...
func TestLinkFFI(t *testing.T) {
	root := &normalize.GASTNode{
		...
		Kind: "function",
		Children: []*normalize.GASTNode{
			{Type: normalize.GASTImport, Name: `"C"`, Kind: "import"},
			{
				Type:       normalize.GASTCallExpression,
				Name:       "C.malloc",
				Kind:       "call",
...
func TestLinkDependencyInjection(t *testing.T) {
	...
	LinkDependencyInjection(aggregateOut, cpg)
...
func TestLinkCrossLanguageRPC(t *testing.T) {
	aggregateOut := &aggregate.AggregateOutput{
		...
		LocalTables: map[string]*normalize.FileSymbolTable{
			"server.go": {
				Endpoints: []normalize.EndpointMeta{{Method: "GET", Route: "/api/users", LineNumber: 3}},
			},
		},
		GlobalCallQueue: []aggregate.LinkedCallSite{
			{
	...
	LinkCrossLanguageRPC(aggregateOut, cpg)
...
func TestLinkConstraintsFullMode(t *testing.T) {
	...
	LinkConstraints(aggregateOut, cpg)
...
func TestLinkAliasAnalysis(t *testing.T) {
	...
	LinkAliasAnalysis(aggregateOut, cpg)
...
func hasEdge(cpg *LinkOutput, src, tgt string, typ RelationshipType) bool {
