...
func CleanupCPG(aggregateOut *aggregate.AggregateOutput, cpg *LinkOutput) {
	if cpg == nil {
		return
	}

	modulePrefix := ""
	if aggregateOut != nil && aggregateOut.WorkspaceCtx != nil {
		modulePrefix = aggregateOut.WorkspaceCtx.ModulePrefix
	}
...
func cleanExtID(id, modulePrefix string) (string, bool) {
	...
	return aggregate.ResolveExternalKey(path), true
}
...
func rewriteNodeIDs(cpg *LinkOutput, renames map[string]string) {
...
func applyProvenanceDefaults(cpg *LinkOutput) {
