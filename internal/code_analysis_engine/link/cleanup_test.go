...
func TestCleanupCPGSelfHealsMangledExtID(t *testing.T) {
	...
	want := aggregate.ResolveExternalKey("internal/errors")
	apiID := want + "::New"
	...
	out := &aggregate.AggregateOutput{
		WorkspaceCtx: &aggregate.WorkspaceContext{ModulePrefix: "github.com/Syamchand123/GlassMarble"},
	}
	CleanupCPG(out, cpg)
...
func TestCleanupCPGStripsModulePrefix(t *testing.T) {
	...
	if _, ok := cpg.GetNode(aggregate.ResolveExternalKey("internal/errors")); !ok {
...
func TestCleanExtID(t *testing.T) {
	cases := []struct {
		...
		{`ext:akgerrs "github.com/Syamchand123/GlassMarble/internal/errors"`, "github.com/Syamchand123/GlassMarble",
			aggregate.ResolveExternalKey("internal/errors"), true},
		{`ext:alias "net/http"`, "", aggregate.ResolveExternalKey("net/http"), true},
		{`ext:old path with spaces`, "", aggregate.ResolveExternalKey("path with spaces"), true},
		{`ext:internal/errors`, "", aggregate.ResolveExternalKey("internal/errors"), true}, // unescaped → self-heal
