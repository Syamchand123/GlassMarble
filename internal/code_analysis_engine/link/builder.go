...
func BuildUniversalID(relPath, receiver, name string) string {
	cleanPath := aggregate.NormalizeRelativePath(relPath)
	if cleanPath == "" {
		cleanPath = "root"
	}
...
// InitialGraphBuilder traverses Aggregation's DirectoryNode topology and registers all vertices into GraphNodes.
type InitialGraphBuilder struct {
	output *LinkOutput
	modSet map[string]bool
}

func (b *InitialGraphBuilder) registerNode(baseID string, node *ResolvedNode, gastNode *normalize.GASTNode) string {
	finalID := baseID
...
func generateASTHash(node *normalize.GASTNode) string {
	if node == nil {
		return "nil"
	}
...
// BuildInitialNodes populates LinkOutput.GraphNodes with Universal Namespace Prefixed IDs for modified files.
func BuildInitialNodes(aggregateOut *aggregate.AggregateOutput, modifiedFiles []string) *LinkOutput {
	output := NewLinkOutput(aggregateOut.CommitHash)

	modSet := make(map[string]bool)
	for _, f := range modifiedFiles {
		modSet[aggregate.NormalizeRelativePath(f)] = true
	}

	builder := &InitialGraphBuilder{output: output, modSet: modSet}

	if aggregateOut.RootNode != nil {
...
func (b *InitialGraphBuilder) traverseDirectory(dir *aggregate.DirectoryNode) {
	if dir == nil {
		return
	}
	...
	if dir.RelativePath != "" && dir.RelativePath != "." {
		modID := "module:" + aggregate.NormalizeRelativePath(dir.RelativePath)
		b.output.GraphNodes[modID] = &ResolvedNode{
			ID:   modID,
			Kind: "MODULE",
			Name: dir.FolderName,
			FileSpec: LocationMeta{
				Path: aggregate.NormalizeRelativePath(dir.RelativePath),
			},
		}
	...
	for _, file := range dir.Files {
		...
		normPath := aggregate.NormalizeRelativePath(file.RelativePath)
...
func (b *InitialGraphBuilder) extractNodesFromGAST(node *normalize.GASTNode, relPath string, parentType string, fileMembers *[]string) {
	if node == nil {
		return
	}
	...
	case normalize.GASTTypeDeclaration:
		...
		resolvedNode := &ResolvedNode{
			...
			// Deep-copy so downstream phases can annotate Properties without
