...
// named children so phase 2 translators never need content-regex parsing
...
// IngestOutput is the final Ingestion payload consumed by Normalization.
...
// Config holds the knobs for a Ingestion ingestion run.
type Config struct {
	RootDir       string
	WorkerCount   int
	MaxFileBytes  int64
