...
func TestStage3OutputStructure(t *testing.T) {
	out := &Stage3Output{
		CommitHash:            "abc123",
		...
		ExternalDependencies:  make(map[string]*stage2.GASTNode),
...
func TestIndexedNode(t *testing.T) {
	in := IndexedNode{
		Key:  "src.main.main",
		Node: &stage2.GASTNode{Name: "main", Type: stage2.GASTFunction},
	}
...
func TestExtractCallsFromFile(t *testing.T) {
	st := &stage2.FileSymbolTable{
		RelPath: "main.go",
		Imports: []string{"fmt"},
		LocalCalls: []stage2.CallSite{
			{
...
func TestComputeVisibilityEnclave(t *testing.T) {
	...
	node := &stage2.GASTNode{
		...
		Visibility: "public",
		Children: []*stage2.GASTNode{
			{Type: stage2.GASTVariable, Name: "localVar", Visibility: "internal"},
		},
	}
