...
// repoRoot resolves the GlassMarble repo root from the normalize test dir
// (internal/code_analysis_engine/normalize → repo root).
...
// RunNormalize writes the given files (relPath → content) into a temp dir and
// runs the full Ingestion → Normalization pipeline over them.
func RunNormalize(t *testing.T, files map[string]string) *NormalizeOutput {
	t.Helper()
	dir := t.TempDir()
...
func treeFor(t *testing.T, payload *NormalizeOutput, slashRel string) *GASTNode {
	t.Helper()
...
func keys(payload *NormalizeOutput) []string {
	var ks []string
