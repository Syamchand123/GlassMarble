...
func (t *GoTranslator) CoerceToken(tok ingest.RichToken, parent *ingest.RichToken, fileRelPath string) *GASTNode {
...
func applySignature(node *GASTNode, tok ingest.RichToken) {
...
func goTypeKind(tok ingest.RichToken) string {
...
func goAliasTarget(tok ingest.RichToken) string {
...
func parseGoTypeParams(tok ingest.RichToken) []TypeParam {
	content := tok.Content
	name := tok.Name
	if name == "" {
