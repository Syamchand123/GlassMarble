...
// and does NOT pollute GASTFileRoot nodes, preserving architectural granularity for whole-program AKG linking.
...
func ApplyEnterpriseIntelligence(node *GASTNode, symTable *FileSymbolTable, RichTokens []ingest.RichToken, nodes []*GASTNode) {
	if node == nil {
		return
	}
...
func hasNPlusOne(node *GASTNode, RichTokens []ingest.RichToken, nodes []*GASTNode) bool {
