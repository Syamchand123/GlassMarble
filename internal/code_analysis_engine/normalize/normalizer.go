...
func Normalize(ingestOut *ingest.IngestOutput, commitHash string) (*NormalizeOutput, error) {
	...
	for _, deleted := range ingestOut.Deleted {
		payload.DeletedPaths = append(payload.DeletedPaths, deleted.RelPath)
	}

	// Filter valid results
	var valid []*ingest.IngestionResult
	for _, res := range ingestOut.Updated {
		if res.Error == nil {
	...
	taskCh := make(chan *ingest.IngestionResult, n)
	...
	go func() {
		defer func() {
			if r := recover(); r != nil {
				log.Printf("normalize: result collector panicked: %v", r)
			}
		}()
		wg.Wait()
		close(resultCh)
...
func ensureFilePaths(payload *NormalizeOutput) {
...
func processFileResult(res *ingest.IngestionResult) (*GASTNode, *FileSymbolTable) {
	...
	for i, tok := range res.RichTokens {
		...
		if node.Type == GASTTypeDeclaration || node.Type == GASTFunction {
			...
			if node.Properties["fully_qualified_name"] == "" {
				...
				// translator/phase populated them; legacy dotted FQNs remain
	...
	// children). Cross-file members are resolved by phase 3 ownership.
...
func parentToken(tok ingest.RichToken, tokens []ingest.RichToken) *ingest.RichToken {
...
func detectPackageName(res *ingest.IngestionResult, nodes []*GASTNode) string {
	for i, tok := range res.RichTokens {
		node := nodes[i]
...
func findEnclosingFunctionID(nodes []*GASTNode, RichTokens []ingest.RichToken, parentIdx int, defaultID string) string {
	curr := parentIdx
...
func extractAdvancedSemantics(node *GASTNode, tok ingest.RichToken, symTable *FileSymbolTable, lang ingest.SupportedLang) {
...
func hasBaseClassRole(lang ingest.SupportedLang) bool {
	switch lang {
	case ingest.LangCSS, ingest.LangHTML, ingest.LangJSON, ingest.LangUnknown:
		return false
	default:
		return true
	}
