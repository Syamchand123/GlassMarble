...
// NewMetrics computes all metrics for a subgraph (convenience wrapper around ComputeAllMetrics).
func NewMetrics(sub *types.VirtualSubgraph, opts types.QueryOptions) *DiagramMetrics {
...
func MetricsToString(metrics *DiagramMetrics) string {
	if metrics == nil {
		return "no metrics"
	}
	s := metrics.Summary
