...
type RuleDefinition struct {
	...
	Name    string
	Tier    string
	Enabled func(node *link.ResolvedNode, graph *CodePropertyGraph, disabledRules map[string]bool, primitivesFound map[string]bool, flags ruleFlags) bool
	Apply   func(node *link.ResolvedNode, graph *CodePropertyGraph, primitivesFound map[string]bool, flags ruleFlags) string
}
	...
	RuleRegistry = []RuleDefinition{
		{
			...
			Enabled: func(n *link.ResolvedNode, g *CodePropertyGraph, dr map[string]bool, pf map[string]bool, f ruleFlags) bool {
				if dr["rule_01"] {
					return false
				}
			...
			Apply: func(n *link.ResolvedNode, g *CodePropertyGraph, pf map[string]bool, f ruleFlags) string {
...
func nodeMacroKey(node *link.ResolvedNode, graph *CodePropertyGraph, macroMode string, disabledRules map[string]bool) string {
	h := sha256.New()
