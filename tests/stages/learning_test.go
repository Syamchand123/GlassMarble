// convention learning (learning) tests: correction log, overlay, pattern feedback,
// conventions.
//
// Discrepancies vs the phase-10 spec: harness.SeedCorrections writes
...
func TestS10LearnConventionsFromGraphAndMemory(t *testing.T) {
	...
	g.Nodes = g.Nodes.Set("internal/services/order.go::OrderService", &link.ResolvedNode{ID: "internal/services/order.go::OrderService", Kind: "STRUCT", Name: "OrderService", FileSpec: link.LocationMeta{Path: "internal/services/order.go"}})
	g.Nodes = g.Nodes.Set("internal/services/billing.go::BillingService", &link.ResolvedNode{ID: "internal/services/billing.go::BillingService", Kind: "STRUCT", Name: "BillingService", FileSpec: link.LocationMeta{Path: "internal/services/billing.go"}})
