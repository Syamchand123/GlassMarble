...
// evidence retrieval (evidence retrieval) tests against a REAL analyzed graph
// (phases 1-4 committed via the AKG transaction manager — see
// pipeline_test.go), a seeded developer-memory aggregate, and a hand-written
// Architecture Intelligence intelligence artifact. Direct API calls only (ai_engine.Retriever).
//
...
// s12WriteIntelligence writes a IntelligenceResult into
// .glassmarble/intelligence/latest.json, the artifact LoadLatestResult reads.
func s12WriteIntelligence(t *testing.T, sb *harness.Sandbox, res arch_intelligence.IntelligenceResult) {
	t.Helper()
	data, err := json.MarshalIndent(res, "", "  ")
	if err != nil {
		t.Fatalf("s12: marshal intelligence result: %v", err)
	}
...
// s12LatestFixture returns the hand-written intelligence result used by the
// seeded-retrieval tests: one LAYERED pattern over the cache (with a
// citable rule reference) and a metrics block.
func s12LatestFixture() arch_intelligence.IntelligenceResult {
	return arch_intelligence.IntelligenceResult{
		GraphHash: "s12hash",
...
// TestEvidenceRetrievalSeededRetrieval drives the full pipeline: real graph committed
// via the transaction manager, seeded memory, hand-written Architecture Intelligence
// intelligence. The retrieval must ground every section.
func TestEvidenceRetrievalSeededRetrieval(t *testing.T) {
...
// TestEvidenceRetrievalNodeMatching asserts every returned node carries the query term
// in its name or file path, and that match scores stay within (0, 1].
func TestEvidenceRetrievalNodeMatching(t *testing.T) {
	sb := harness.NewSandbox(t)
	analyzeProject(t, sb)
...
// TestEvidenceRetrievalRetrievalDeterminism asserts two retrievals of the same question
// yield byte-identical node and claim orderings.
func TestEvidenceRetrievalRetrievalDeterminism(t *testing.T) {
	sb := harness.NewSandbox(t)
	analyzeProject(t, sb)
	sb.SeedMemory("proj_test")
	s12WriteIntelligence(t, sb, s12LatestFixture())
...
// TestEvidenceRetrievalTopKCapsSections asserts TopK bounds every evidence section.
func TestEvidenceRetrievalTopKCapsSections(t *testing.T) {
	sb := harness.NewSandbox(t)
	analyzeProject(t, sb)
	sb.SeedMemory("proj_test")
	s12WriteIntelligence(t, sb, s12LatestFixture())
...
// TestEvidenceRetrievalTokenBudgetTrim asserts TrimToBudget pulls the rendered prompt
...
func TestEvidenceRetrievalTokenBudgetTrim(t *testing.T) {
	sb := harness.NewSandbox(t)
	analyzeProject(t, sb)
	sb.SeedMemory("proj_test")
	s12WriteIntelligence(t, sb, s12LatestFixture())
...
// TestEvidenceRetrievalEmptyInputs asserts empty and whitespace questions short-circuit
// to an empty context without error.
func TestEvidenceRetrievalEmptyInputs(t *testing.T) {
	sb := harness.NewSandbox(t)
	sb.SampleProject()
	sb.SeedMemory("proj_test")
...
// TestEvidenceRetrievalFreshSandboxNoPanic asserts retrieval against a completely empty
// sandbox (no .glassmarble at all) yields an empty context, no panic and no
// error.
func TestEvidenceRetrievalFreshSandboxNoPanic(t *testing.T) {
	sb := harness.NewSandbox(t)
...
// TestEvidenceRetrievalMissingPieces asserts retrieval degrades gracefully when only
// one data source exists: graph without memory yields nodes only, memory
// without graph yields claims only.
func TestEvidenceRetrievalMissingPieces(t *testing.T) {
...
// TestEvidenceRetrievalCorrectionRejectsPattern asserts a REJECT correction whose
// PATTERN_DETECTED event names a pattern kind excludes that kind from the
// evidence while a sibling pattern survives.
func TestEvidenceRetrievalCorrectionRejectsPattern(t *testing.T) {
	sb := harness.NewSandbox(t)
	analyzeProject(t, sb)
...
// TestEvidenceRetrievalSnapshotFallback asserts retrieval still grounds on the latest
...
func TestEvidenceRetrievalSnapshotFallback(t *testing.T) {
	sb := harness.NewSandbox(t)
	sb.WriteAKGState(harness.TinyGraph())
