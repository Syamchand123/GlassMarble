...
// Note: ingest.Config has no WithWorkers method (the API reference mentions
// one); tests set cfg.WorkerCount directly instead.

// updatedPaths returns the set of slash-normalized RelPaths in a phase 1
// output (the walker emits native separators on Windows).
func updatedPaths(out *ingest.IngestOutput) map[string]bool {
...
func TestIngestionRunIngestionSampleProject(t *testing.T) {
...
func TestIngestionConfigTuning(t *testing.T) {
	sb := newSampleSandbox(t)
	...
	if err != nil {
		t.Fatalf("ingest.RunIngestion with 2 workers: %v", err)
	}
...
func TestIngestionCollectGitDiffWorkingTree(t *testing.T) {
	sb := newSampleSandbox(t)
	sb.RequireGit()
	sb.GitInit()
	...
	tasks, err := ingest.CollectGitDiff(sb.Root, "")
	if err != nil {
		t.Fatalf("ingest.CollectGitDiff dirty tree: %v", err)
	}
	found := false
	for _, task := range tasks {
		if task.RelPath == "cmd/api/extra.go" {
			found = true
			if task.Change != ingest.ChangeAdded {
				t.Errorf("extra.go change = %q, want %q", task.Change, ingest.ChangeAdded)
			}
		}
	}
	if !found {
...
func TestIngestionCollectGitDiffCommit(t *testing.T) {
	...
	tasks1, err := ingest.CollectGitDiff(sb.Root, first)
	if err != nil {
		t.Fatalf("ingest.CollectGitDiff(first commit): %v", err)
	}
	...
	tasks2, err := ingest.CollectGitDiff(sb.Root, second)
	if err != nil {
		t.Fatalf("ingest.CollectGitDiff(second commit): %v", err)
	}
	...
	if tasks2[0].Change != ingest.ChangeModified {
		t.Errorf("cache.go change = %q, want %q", tasks2[0].Change, ingest.ChangeModified)
	}
}

func TestIngestionRunIngestionForDelta(t *testing.T) {
	sb := newSampleSandbox(t)
	sb.RequireGit()
	sb.GitInit()
	sb.GitCommit("feat: sample project")
	...
	diff, err := ingest.CollectGitDiff(sb.Root, second)
	if err != nil {
		t.Fatalf("ingest.CollectGitDiff: %v", err)
	}

	out, err := ingest.RunIngestionForDelta(ingest.DefaultConfig(sb.Root), diff)
	if err != nil {
		t.Fatalf("ingest.RunIngestionForDelta: %v", err)
	}
...
func TestIngestionUnknownLanguageHandled(t *testing.T) {
	sb := newSampleSandbox(t)
	sb.WriteFile("notes/readme.xyz", "hello unknown world\n")

	out, err := ingest.RunIngestion(ingest.DefaultConfig(sb.Root))
	if err != nil {
		t.Fatalf("ingest.RunIngestion with unknown-language file: %v", err)
	}
	if updatedPaths(out)["notes/readme.xyz"] {
		t.Errorf("unknown-language file must not be parsed, got entry in Updated")
	}

	delta, err := ingest.RunIngestionForDelta(ingest.DefaultConfig(sb.Root), []ingest.FileTask{{
		FilePath: sb.Path("notes/readme.xyz"),
		RelPath:  "notes/readme.xyz",
		Change:   ingest.ChangeAdded,
	}})
	if err != nil {
		t.Fatalf("ingest.RunIngestionForDelta with unknown-language task: %v", err)
	}
