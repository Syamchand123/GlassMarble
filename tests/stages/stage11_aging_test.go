// Stage 11 (knowledge aging) tests: freshness decay, Ager transitions,
// stale-entity detection.
//
// Discrepancies vs the stage-11 spec: there is no free function
...
func TestS11PinnedComponentNotTransitioned(t *testing.T) {
	...
	if len(results) != 0 {
		t.Errorf("transitions = %d, want 0 (Stage 10 pin is authoritative)", len(results))
	}
