...
func findNode(root *normalize.GASTNode, pred func(*normalize.GASTNode) bool) *normalize.GASTNode {
	if root == nil {
		return nil
	}
	if pred(root) {
...
func TestNormalizeNormalizeBuildsTrees(t *testing.T) {
	sb := newSampleSandbox(t)
	_, payload, _, _ := runAnalysisPipeline(t, sb, "cafe1234cafe1234")
...
func TestNormalizeNormalizeMainTreeHasFunctions(t *testing.T) {
	sb := newSampleSandbox(t)
	...
	if root.Type != normalize.GASTFileRoot {
		t.Errorf("main.go root Type = %q, want %q", root.Type, normalize.GASTFileRoot)
	}
	if len(root.Children) == 0 {
		t.Fatalf("main.go root has no children, want functions")
	}
	main := findNode(root, func(n *normalize.GASTNode) bool {
		return n.Type == normalize.GASTFunction && n.Name == "main"
	})
	if main == nil {
...
func TestNormalizeNormalizeNilTolerance(t *testing.T) {
	payload, err := normalize.Normalize(nil, "hash")
	if err != nil {
		t.Fatalf("normalize.Normalize(nil, hash): %v", err)
	}
	if payload == nil {
...
func TestNormalizeMultiLanguageDispatch(t *testing.T) {
	sb := newSampleSandbox(t)
	_, payload, _, _ := runAnalysisPipeline(t, sb, "hash")
	...
	if _, ok := normalize.Dispatcher(ingest.LangPython).(*normalize.PythonTranslator); !ok {
		t.Error("Dispatcher(LangPython) is not a *PythonTranslator")
	}
	if _, ok := normalize.Dispatcher(ingest.LangJS).(*normalize.JSTranslator); !ok {
		t.Error("Dispatcher(LangJS) is not a *JSTranslator")
	}
	if _, ok := normalize.Dispatcher(ingest.LangGo).(*normalize.GoTranslator); !ok {
