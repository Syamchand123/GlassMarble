...
// TestFusion_EndToEnd verifies the knowledge fusion wiring: `gmb analyze
...
func TestFusion_EndToEnd(t *testing.T) {
	...
	if !strings.Contains(output, "Fusion: fused") {
		t.Errorf("analyze output missing the knowledge fusion summary:\n%s", output)
	}
...
// TestFusion_FlagDefaultsOff verifies --include-docs is opt-in: a plain
// `gmb analyze` must not run knowledge fusion (no claims WAL is created).
func TestFusion_FlagDefaultsOff(t *testing.T) {
	root := setupAnalyzeGitRepo(t)
	adrDir := filepath.Join(root, "docs", "adr")
	if err := os.MkdirAll(adrDir, 0o755); err != nil {
		t.Fatal(err)
	...
	if strings.Contains(output, "Fusion: fused") {
		t.Errorf("knowledge fusion ran without --include-docs:\n%s", output)
	}
