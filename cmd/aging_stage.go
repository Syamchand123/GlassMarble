...
// runAgingStage is the Stage 11 wiring point in the analysis pipeline
...
func runAgingStage(storageDir string, verbose bool) {
	...
	if !cfg.AgingEnabled() {
		if verbose {
			fmt.Println("Stage 11: knowledge aging disabled by config (aging.enabled=false)")
		}
		return
	}
	...
	if aerr != nil {
		fmt.Printf("warning: Stage 11 knowledge aging failed: %v\n", aerr)
		return
	}
	...
	if err != nil {
		if verbose {
			fmt.Printf("warning: Stage 11 could not load memory for summary: %v\n", err)
		}
		return
	}
	avg, claims := averageFreshness(mem, now)
	if len(transitions) > 0 {
		fmt.Printf("Stage 11: %d state transition(s) | %d claim(s) aged | average freshness %.0f%%\n",
			len(transitions), claims, avg*100)
		for _, tr := range transitions {
			fmt.Printf("  %s: %s → %s (%s)\n", tr.Component, tr.OldState, tr.NewState, tr.RuleID)
		}
	} else if verbose {
		fmt.Printf("Stage 11: no state transitions | %d claim(s) aged | average freshness %.0f%%\n",
			claims, avg*100)
	}
}

// agingPinsFromCorrections derives the Stage 11 pin set from the Stage 10
...
func agingPinsFromCorrections(repoDir string, store *developer_memory.MemoryStore, verbose bool) map[string]developer_memory.KnowledgeState {
	...
	if len(pins) > 0 && verbose {
		...
		for _, name := range names {
			fmt.Printf("Stage 11: component %q pinned to %s by a STATE correction (aging will not transition it)\n",
				name, pins[name])
		}
	}
	return pins
