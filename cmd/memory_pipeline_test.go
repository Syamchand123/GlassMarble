...
// TestMemoryPipeline_EndToEnd verifies the intelligence + memory wiring: `gmb
...
func TestMemoryPipeline_EndToEnd(t *testing.T) {
	root := setupAnalyzeGitRepo(t)

	output, err := runGmbCommand(t, "analyze", "--dir", root, "--intelligence")
	if err != nil {
	...
	// latest.json must be valid JSON with phase 5 output.
...
// TestMemoryPipeline_CLIWithSeededMemory drives the `gmb memory` CLI against a
// real on-disk memory seeded through the developer_memory builder (the same
// store the pipeline writes to).
func TestMemoryPipeline_CLIWithSeededMemory(t *testing.T) {
	root := setupAnalyzeGitRepo(t)
...
// TestMemoryPipeline_InitCreatesStageDirs verifies `gmb init` provisions the V2
// phase directories.
func TestMemoryPipeline_InitCreatesStageDirs(t *testing.T) {
	root := t.TempDir()
	if _, err := runGmbCommand(t, "init", "--dir", root); err != nil {
		t.Fatalf("init failed: %v", err)
	}
